...
getwd()
setwd('S:/Air_Quality_Pearce/Boz_Diss/Results/aim3/')
...
model.code<-nimbleCode({
  ...
  {	
    for (j in 1:4) # index case day (1) and control days (3)
    {
      ...
      logit(pi[i,j]) <- a0 + 
        ...
        b5*MPM25c[i,j] +
        #b6*MSO2c[i,j] + 
        b7*Mtempc[i,j] + b9*Mtedec[i,j] + # b8*Mdewpc[i,j] +
        c[combos[i]] 
  ...
  b4~dnorm(0,taub4); taub4~dgamma(2,1) # PMx (changed from PM10)
...
# load ##################################################################################################################
load(file = "path/constants_SC_summer.rda") # constants/data

load(file = "path/outcome_SC_summer.rda") # outcome

load(file = "path/inits_SC_summer.rda") # initial values
# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
